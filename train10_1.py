...
from ViT_patchloss7 import PairContrastiveLoss
...
output1 = nn.Sequential(
    nn.Conv2d(2, 1, 1, 1, 0, bias=True),
    nn.Sigmoid()).to(device)
#output2 = nn.Sequential(
#    nn.Conv2d(3, 1, 1, 1, 0, bias=True),
#    nn.Sigmoid()).to(device)
#teacher.load_state_dict(student.state_dict())
criterion1 = PairContrastiveLoss(2, 8, 4, latent=512)
...
for epoch in range(300):
    ...
    if (epoch+1) % 1 == 0:
        ...
        for x0, y0 in valid_loader:
            ...
            with torch.no_grad():
                output0 = output1(head1(student(x0)))
