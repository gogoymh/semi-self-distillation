...
optimizer_s = optim.Adam(student.parameters(), lr=5e-4)
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer_s, T_max=50)
ema_tau = 0.01
...
alpha = 1
#beta = torch.linspace(0.1, 2, steps=300)
#beta = 0.1
...
for epoch in range(300):
    ...
    for x1, _ in unlabeled_loader:
        ...
        output0 = student(x0)
        loss_sup = criterion(output0, y0)
        
        _ = teacher(x0)
        '''
        output1 = student(x1)
        #output1 = T(output1, seed)
        
        output2 = teacher(x1)#2)
        output2 = output2.detach()
        
        loss_self = criterion(output1, output2)
        '''
        ...
        running_loss2 += 0#loss_self.item()
        #print(loss.item())
