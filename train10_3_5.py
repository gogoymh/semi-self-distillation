...
param2 = list(student.parameters()) + list(output1.parameters())
...
#incre = nn.AvgPool2d(3,1,1)
class labeling(nn.Module):
    def __init__(self):
        super().__init__()
        
        #self.incre = nn.AvgPool2d(3,1,1)
        self.incre = nn.MaxPool2d(7,1,3)
        
    def forward(self, x, delta):
        x = (1-delta) * self.incre(x) + delta * x
        return x

delta = ((-torch.exp(torch.linspace(1,0,300))+torch.exp(torch.ones(1)))/(-1+torch.exp(torch.ones(1)))).to(device)
incre = labeling().to(device)
...
for epoch in range(300):
    ...
    for x1, _ in unlabeled_loader:
        ...
        if epoch >= 5:
            ...
            rep1_s = student(x1)
            output1_s = output1(rep1_s)
            
            prob = incre(output1_s, delta[epoch]).detach()            
            prob = torch.cat((prob,(1-prob)), dim=1)
            prob = prob.permute(0,2,3,1).reshape(-1,2)
            pseudo_y1 = torch.multinomial(prob, 1).reshape(B,1,224,224)
            
            loss_self = criterion2(output1_s, pseudo_y1) 
            #loss += loss_self
            loss_self.backward()
            optimizer2.step()
        ...
        loss_sup = criterion2(output0_s, y0)
        #loss += loss_sup
        loss_sup.backward()
        optimizer2.step()
