...
from ViT_patchloss14 import PairContrastiveLoss
...
for epoch in range(300):
    ...
    for x1, _ in unlabeled_loader:
        ...
        y1 = (output2(teacher(x1)) >= 0.5).float().detach()
        #y1 = (output1(teacher(x1)) >= 0.5).float().detach()

        loss_self = criterion1(rep1_s, y1) + criterion2(output1_s, y1)
        ...
        for target_param, param in zip(output2.parameters(), output1.parameters()):
            #target_param.data.copy_(target_param.data * (1.0 - ema_tau[epoch]) + param.data * ema_tau[epoch])
            target_param.data.copy_(target_param.data * (1.0 - ema_tau) + param.data * ema_tau)
