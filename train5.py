...
from ViT_patchloss2 import PairContrastiveLoss

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

student = net().to(device)
#teacher = net().to(device)
head1 = nn.Conv2d(32, 3, 1, 1, 0, bias=True).to(device)
...
criterion1 = PairContrastiveLoss(3, 768, 16)
...
param = list(student.parameters()) + list(head1.parameters())# + list(teacher.parameters()) # + list(criterion1.parameters())# + list(head2.parameters())
...
for epoch in range(300):
    ...
    for x1, _ in unlabeled_loader:
        ...
        #output1_t = head1(rep1_t)

        loss_self = criterion1(output1_s, output1_s)# output1_t)#.detach()) + criterion1(output1_t, output1_s.detach())
    ...
    print("[Epoch:%d][Loss self:%f]" % ((epoch+1), running_loss2))

for epoch in range(300):
    running_loss1 = 0
    running_loss2 = 0
    for x0, y0 in labeled_loader:
        ## ---- supervised ---- ##
        ...
        #output0_t = output2(rep0_t)
        
        loss_sup_1 = criterion2(output0_s, y0)# + criterion2(output0_t, y0)
        loss_sup_1.backward()
        #loss_sup_2 = criterion2(output0_t, y0)
        #loss_sup_2.backward()
        
        optimizer2_1.step()
        #optimizer2_2.step()
        
        running_loss1 += loss_sup_1.item()
        #running_loss2 += loss_sup_2.item()
            
    running_loss1 /= len(labeled_loader)
    running_loss2 /= len(labeled_loader)
    print("="*100)
    print("[Epoch:%d] [Loss sup1:%f] [Loss sup2:%f]" % ((epoch+1), running_loss1, running_loss2))
