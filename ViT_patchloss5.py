...
class PairContrastiveLoss(nn.Module):
    ...
    def forward(self, x, y, eps):
        ...
        close_bool = SIM > eps
        #close_bool[:, eye.long().type(torch.bool)] = False
        
        far_bool = ~close_bool#SIM < -eps
        #close_bool[:, eye.long().type(torch.bool)] = False
        far_bool[:, eye.long().type(torch.bool)] = False
        
        close = SIM[close_bool]
        far = SIM[far_bool]
