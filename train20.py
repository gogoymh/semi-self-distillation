...
optimizer1 = optim.Adam(params1, lr=5e-4)
optimizer2 = optim.Adam(params2, lr=5e-4)
...
#beta = torch.cat((torch.zeros(1), torch.linspace(0, 3.75, steps=299)), dim=0)
beta = torch.linspace(0, 3.75, steps=300)
...
for epoch in range(300):
    ...
    model1.train()
    model2.train()
    for x1_1, _ in unlabeled_loader1:
        ## ---- model1 ---- ##
        ...
        output0_0_1 = model0(x0_1)
        output1_0_1 = model1(x0_1)
        loss_sup1 = criterion(output0_0_1, y0_1) + criterion(output1_0_1, y0_1)
        
        output0_1_1 = model0(x1_1)
        output1_1_1 = model1(x1_1)
        loss_self1 = criterion(output0_1_1, (output1_1_1 >= 0.5).detach()) + criterion(output1_1_1, (output0_1_1 >= 0.5).detach())
        
        loss1 = loss_sup1 + beta[epoch] * loss_self1
        loss1.backward()
        optimizer1.step()
        
        running_loss1 += loss_sup1.item()
        running_loss2 += loss_self1.item()
    
    for x1_2, _ in unlabeled_loader1:
        ## ---- model2 ---- ##
        ...
        output0_0_2 = model0(x0_2)
        output2_0_2 = model2(x0_2)
        loss_sup2 = criterion(output0_0_2, y0_2) + criterion(output2_0_2, y0_2)
        
        output0_1_2 = model0(x1_2)
        output2_1_2 = model2(x1_2)        
        loss_self2 = criterion(output0_1_2, (output2_1_2 >= 0.5).detach()) + criterion(output2_1_2, (output0_1_2 >= 0.5).detach())
        
        loss2 = loss_sup2 + beta[epoch] * loss_self2
        loss2.backward()
        optimizer2.step()
        
        running_loss1 += loss_sup2.item()
        running_loss2 += loss_self2.item()
        #print(loss.item())
